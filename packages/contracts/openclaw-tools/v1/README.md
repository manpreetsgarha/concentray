...
- `task_get_next` -> `task get-next --assignee ... --status ... --json`
- `task_claim_next` -> `task claim-next --worker-id ... --assignee ... --status ... --json`
...
OpenClaw should use `task_claim_next` when it intends to start work. `task_get_next` is for read-only queue inspection. If `worker_id` is omitted, the wrapper derives one from `OPENCLAW_WORKER_ID`, `TM_WORKER_ID`, or the local hostname.
